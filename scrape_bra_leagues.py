...
def scrape_league(league_key: str, mode: str = "all", engine=None) -> dict:
    """
    ...
    try:
        if mode in ["all", "outfield"]:
            ...
            df.insert(0, 'data_execucao', data_execucao)
            df.insert(1, 'league', league_name)
            df['scraped_at'] = scraped_at
            
            if engine:
                print(f"    → [SQL] Inserindo players_outfield no banco...")
        ...
        if mode in ["all", "keepers"]:
            ...
            if engine:
                print(f"    → [SQL] Inserindo players_keepers no banco...")
        ...
        print(f"  ✗ Erro: {e}")
        return {}
def run_pipeline(league_to_run=None, mode="all", to_db=False):
    ...
    for l_key in leagues:
        scrape_league(l_key, mode, engine=engine)

if __name__ == "__main__":
    # Executa o pipeline completo enviando para o banco por padrão
    run_pipeline(to_db=True)
